...
import { createProduct, updateProduct } from "@/lib/api/materials";
...
interface ProductFormSheetProps {
  ...
  /**
   * The caller's bound local_field_id when the session is LF-scoped. Used to
   * hide the LF picker (auto-fills the form). null = unscoped admin who must
   * pick.
   */
...
function CreateForm({
  ...
  const form = useForm<CreateFormValues>({
    ...
    defaultValues: {
      ...
      title: "",
      description: "",
      // LF-scoped users get their LF preloaded; unscoped admins start empty.
      local_field_id: actorLocalFieldId ?? undefined,
  ...
  async function onSubmit(values: CreateFormValues) {
    try {
      await createProduct(
        ...
        { localFieldId: values.local_field_id },
      );
  ...
  return (
    <Form {...form}>
      <form
        ...
        className="space-y-4"
      >
        {/* Local field — only when caller is unscoped admin (localFields populated) */}
