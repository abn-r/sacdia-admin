...
import {
  buildV2CoordinatorLfNavConfig,
  buildV2NavConfig,
} from "@/navigation/v2/nav-config";
    ...
    <SidebarMenuSubItem>
      <SidebarMenuSubButton asChild isActive={pathname === child.url}>
        <Link href={child.url} prefetch={false}>
          <span className="truncate" title={childTitle}>
            {childTitle}
          </span>
        </Link>
      </SidebarMenuSubButton>
...
function NavItemWithChildren({
  ...
  const grouped =
    item.children && item.children.length > 0 && isSubGroup(item.children[0]);
          ...
          <SidebarMenuSub>
            {grouped
              ? (item.children as NavSubGroup[]).map((sg) => {
                  ...
                  const labelId = `v2-sidebar-subgroup-${item.url.replace(/[^a-z0-9]/gi, "-")}-${sg.subgroup}`;
                  return (
                    <li
                      key={sg.subgroup}
                      role="group"
...
function NavItemSimple({ item, pathname }: { item: NavItem; pathname: string }) {
  ...
  const isActive =
    pathname === item.url ||
    (item.url !== "/v2/dashboard" && pathname.startsWith(item.url + "/"));
  const title = t(item.title as Parameters<typeof t>[0]);
          ...
          <DropdownMenu>
            ...
            >
              <DropdownMenuItem asChild>
                <Link
                  href={toV2Path(`/dashboard/users/${user?.id ?? ""}`)}
                  prefetch={false}
                >
        ...
        <SidebarMenu>
          <SidebarMenuItem>
            <SidebarMenuButton size="lg" asChild>
              <Link href="/v2/dashboard" prefetch={false}>
