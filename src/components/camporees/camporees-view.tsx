...
interface CampoReesViewProps {
  initialCamporees: Camporee[];
}
...
export function CampoReesView({ initialCamporees }: CampoReesViewProps) {
  ...
  const [deletingCamporee, setDeletingCamporee] = useState<Camporee | null>(null);

  const refreshList = useCallback(async () => {
    setIsLoading(true);
    setLoadError(null);
    try {
      const payload = await listCamporees();
